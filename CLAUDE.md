...
Home Screen:
- `home-default-sections-location` | Mobile sections in Home.tsx (HomeMobile); TV separate in Home.tv.tsx
- `jellyfin-getsuggestions-movie-only` | getSuggestions returns [] for Series; use getItems + IsFavoriteOrLiked,Random
- `hideifempty-infinite-scrolling-collection-list` | Empty query + hideIfEmpty silently hides section, no empty state
- `streamystats-gates-default-recommendation-rows` | StreamyStats recs suppress Suggested rows; custom home.sections replaces all defaults
